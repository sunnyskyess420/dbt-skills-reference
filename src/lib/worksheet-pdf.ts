...
function generateGenericWorksheet(doc: any, entry: WorksheetEntry) {
  ...
  // Use the shared helpers — they manage positioning and page breaks via the
  // module-level `y` (already reset to PAGE_MARGIN by exportToPdf). The old
  // version passed unused layout args that shifted `label` into `num`, so
  // section titles rendered as "SomeKey. 20" instead of the real label.
  writeTitle(doc, entry.title);
  writeSubtitle(doc, `${meta.name} — ${meta.reference}`);

  const keys = Object.keys(d).filter(
    (k) => d[k] !== "" && d[k] !== 0 && d[k] !== false && d[k] !== undefined
  );
  for (const key of keys) {
    const label = key
      .replace(/([A-Z])/g, " $1")
      .replace(/^./, (s) => s.toUpperCase());
    writeSectionTitle(doc, undefined, label);
    writeValue(doc, String(d[key]));
    y += 4;
  }
}
