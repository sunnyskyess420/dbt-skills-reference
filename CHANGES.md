...
A site review flagged four issues; all four were addressed (details and
measurements in `docs/quality-pass.md`).

1. **Attribution & provenance.** New `/about` page and `NOTICE.md`: source
   attribution (Linehan 2014 / Guilford Press), explicit non-affiliation, medical
   disclaimer, a link to buy the official workbook, and a takedown route. Linked
   from the sidebar footer and help dialog; README license section expanded.
   *This documents provenance — it does not grant any permission.*
2. **Performance.** jsPDF (the 934 KB chunk) no longer loads on first paint:
   PDF export is dynamically imported on click. **Home initial JS 1,599 KB →
   1,171 KB (−27%).** Removed a dead `worksheet-pdf` import in `session-prep.tsx`.
3. **Shareability & SEO.** Real `og:image` (1200×630, generated by
   `tools/make-og-image.mjs`), `app/sitemap.ts`, `app/robots.ts`, and
   `metadataBase` + `twitter:card=summary_large_image`.
4. **Accessibility.** "Skip to content" link plus `id="main-content"` on the main
   landmarks.

**New:** `src/app/about/`, `src/app/sitemap.ts`, `src/app/robots.ts`,
`tools/make-og-image.mjs`, `public/og-image.png`, `NOTICE.md`, `docs/quality-pass.md`.
**Changed:** `src/app/layout.tsx`, `src/app/page.tsx`, `src/components/dbt/sidebar.tsx`,
`src/components/dbt/help-dialog.tsx`, `src/components/dbt/goals.tsx`,
`src/components/dbt/session-prep.tsx`, `src/components/dbt/worksheets/worksheet-detail.tsx`,
`src/components/dbt/worksheets/diary-comparison.tsx`, `README.md`.
No DBT content changed.
